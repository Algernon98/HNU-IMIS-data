...
for i in range(len(points_coordinate)):
    ls = [float(points_coordinate[i][0]), float(points_coordinate[i][1])]
    data.append(ls)
data = np.array(data)
...
print("规划路径:\n", Best_path)   # 返回的是该回路的点的坐标
print('规划路径长度:{}'.format(Best))
